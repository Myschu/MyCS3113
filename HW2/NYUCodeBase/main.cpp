...
class Ball {
public:
	Ball(float posX, float posY, float vel, float spd, float acc, float dirX, float dirY) : pos_x(posX), pos_y(posY), speed(spd),  dx(dirX), dy(dirY) {}
	Ball() {}
	float pos_x = 0.0f;
	float pos_y = 0.0f;
	float speed = 0.1f;
...
int main(int argc, char *argv[])
{
	...
	Paddle leftpaddle = Paddle(-1.65, 0);
	Paddle rightpaddle = Paddle(1.65, 0);
	Ball ball = Ball();
	...
	while (!done) {
		...
		if (keys[SDL_SCANCODE_W] && leftpaddle.pos_y < 0.75f) {
			leftpaddle.pos_y += elapsed * 0.2f;
		}
		else if (keys[SDL_SCANCODE_S] && leftpaddle.pos_y > -0.75f) {
			leftpaddle.pos_y += elapsed * -0.2f;
		}

		if (keys[SDL_SCANCODE_UP] && rightpaddle.pos_y < 0.75f) {
			rightpaddle.pos_y += elapsed * 0.2f;
		}
		...
		if (start) {
			int p1 = (abs(leftpaddle.pos_x - ball.pos_x)) - 0.1;
			int p2 = (abs(leftpaddle.pos_y - ball.pos_y)) - 0.3;
			int p3 = (abs(rightpaddle.pos_x - ball.pos_x)) - 0.1;
			int p4 = (abs(rightpaddle.pos_y - ball.pos_y)) - 0.3;
			...
			else if (ball.pos_y <-0.95 || ball.pos_y > 0.95) {
				ball.dy *= -1;
				ball.pos_x += (ball.speed * ball.dx * elapsed);
				ball.pos_y += (ball.speed * ball.dy * elapsed);
			}
